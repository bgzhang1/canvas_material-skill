...
无论是首次更新还是增量更新，只要某门课完成了下载，就要在“转 PDF”和“分类移动”之间执行自动去重。

规则如下：

- 如果下载到重复文件，只保留一份，其余删除
- 如果某个原始文件已经存在对应 PDF 版本，并且该原始文件只是重复副本，则删除该重复原文件
- 去重应发生在：
  1. 下载后
  2. 按需转 PDF 后
  3. 分类移动前

也就是说，进入分类阶段的应该是**已经过一次去重后的文件集**。

### 去重阶段要求

- 不要把明显重复的文件继续移动到分类目录
- 对“同名重复”与“转换后已存在 PDF 的重复原文件”都要处理
- 增量更新也必须执行同样的去重规则
- 如果用户要求保留所有原件，则应先确认再跳过去重删除动作
